...
ITEMS_TO_IGNORE = [] # We will ignore items such as trinkets & potions
...
def getScore(player,role):
    ...
    for role_item in role:
        ...
        item_id = int(role_item.keys()[0])

        if item_id in player['items']:
            score += role_item[item_id]
...
def getClusters(iteration):
    ...
    roles = readEntireFile('/home/gary/League-New-Meta/newmeta/jsons/kmeans/role_data_{i}.json'.format(i=iteration))
...
def generateNextIteration(iteration):
    ...
    clusters = getClusters(iteration)
